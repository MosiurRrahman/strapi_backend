...
const Home1solutionSection = ({style="home1-solution-section"}) => {
  ...
  const [solutionAccrodionData, setsolutionAccrodionData] = useState({})
  useEffect(() => {
    getsolutionData()
    getsolutionAccrodionData()
  }, [])
  const getsolutionData = () => {
    GlobalApi.getSolution().then((res) => {
      setsolutionData(res.data?.data?.attributes?.solutions)
    })
  }
  const getsolutionAccrodionData = () => {
    GlobalApi.getSolutionAccrodion().then((res) => {
      setsolutionAccrodionData(res.data?.data?.attributes?.solutions)
    })
  }
