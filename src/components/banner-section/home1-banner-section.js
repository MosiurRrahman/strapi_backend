...
const Home1BannerSection = () => {
  ...
  const [partnerLogo, setPartnertLogo] = useState({})
  useEffect(() => {
    getHerodata()
    getPartnerLogo()
  }, [])
  const getHerodata = () => {
    GlobalApi.getHero().then((res) => {
      setBannerData(res.data.data.attributes.hero[0])
    })
  }
  const getPartnerLogo = () => {
    GlobalApi.getPartnerLogo().then((res) => {
      setPartnertLogo(res.data?.data?.attributes?.partner_section || []); // Fallback to empty array
    });
  };


  const { sub_title, success_rate, title, video_link, image,description,link } = bannerData;
  console.log("banner data", link);
          ...
          <div className="row">
            <div className="col-lg-12">
              <div className="banner-main-title">
                <h6 className="text-animation">{sub_title}
                ...
                <h1 className="text-animation">{title && (
                  <>
            ...
            <div className="row">
              <div className="col-lg-7 d-lg-flex d-none">
                <div className="banner-img magnetic-item">
                  <img src={baseUrl + image?.data?.attributes?.url} alt="" />
                </div>
              </div>
                ...
                <div className="banner-right-content">
                  <div className="content">
                    ...
                    <ul className="btn-group">
                      <li className="success-rate">Success Rate <span>{success_rate}%</span>
                      </li>
                      <li>
            ...
            <div className="col-lg-12">
              <div className="partner-wrap">
                    ...
                    <Marquee>
                      {partnerLogo.length > 0 ? (
                        partnerLogo.map((partner, index) => {
                          const imageUrl = partner?.image?.data?.attributes?.url;
                          return (
                            imageUrl && (
                              <a href="#" key={index}>
                                <img
                                  src={`${baseUrl}${imageUrl}`} // Replace with your actual base URL
                                  alt={partner?.image?.data?.attributes?.alternativeText || 'Partner Logo'}
                                />
                              </a>
                            )
                          );
                ...
                <div className="title2">
                  <h6>Almost 20s+ Partner we have</h6>
                </div>
              </div>
            </div>
          </div>
