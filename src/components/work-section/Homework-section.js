...
const HomeworkSection = () => {
  ...
  const getworktData = () => {
    GlobalApi.getWorkSection().then((res) => {
      setworkData(res.data?.data?.attributes?.work_section)
    })
  }
  const getWrokCardDara = () => {
    GlobalApi.getWorkSectionCard().then((res) => {
      setworkCard(res?.data?.data?.attributes?.work_section)
    })
  }
  const { name, description, worok_card } = workCard;
      ...
      <div className="work-section">
        <div className="container">
          ...
          <div className="row mb-80">
            <div className="col-lg-12">
              {/* {
                worok_card.map((card, index) => {
                  console.log(index)
                  
                  return <>
                  <div className={`"row mb-80 g-lg-0 gy-5 justify-content-between "`}></div>
                  </>
                })
              } */}
              {Array.isArray(worok_card) &&
                worok_card.map((e, index) => {
                 
                  
                  return <div key={index} className={`row mb-80 g-lg-0 gy-5 justify-content-between `}>
                    <h3>{e.title}</h3>
                    <p>{e.description}</p>
                    {/* Other content based on the structure of worok_card */}
                  </div>
                })}
